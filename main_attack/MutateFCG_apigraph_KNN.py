...
save_dir = '/data/c/shiwensong/project/apigraph_test_flowdroid/knn_1/'  # 最后save的文件夹
recover_dir = './mamadroid_test_new_mutation_with_shap/knn_1/'  # 最后save的文件夹
shap_path = ''
init_num = 100
...
def ga(fcg, max_generation=100, pop_num=100, steps=500, init_state=False, population=None, sensitive=True, sub = True, group = False, distance = True):
    ...
    if not init_state:
        ...
        pop_label_list, pop_score_list, mlp_pop_list, pop_shap_list = _population_score(fcg, population, False, sub, distance)
    ...
    for i in range(max_generation):
        ...
        if i == max_generation - 1:
            save_state = True
        cur_pop_label_list, cur_pop_score_list, cur_mlp_pop_list, cur_pop_shap_list = _population_score(fcg,
                                                                                                        next_gene_population,
                                                                                                        save_state, sub,
                                                                                                        distance)
...
def _fitness_function(pop_score_list):
    ...
    for i in range(len(pop_score_list)):
        ...
        if pop_score_list[i] is not None and pop_score_list[i] <= 0.0:
            target_index = i
            break
    return target_index


def _population_score(fcg, pop, save_state=False, sub=True, distance=True):
    # version1: shap+score
    score_list = []
    shap_list = []
    MLP_score_list = []
    ...
    # print("y_label_list", y_label_list)
...
def sort_all_pop(pop, pop_score_list, pop_mlp_list, pop_shap_list, pop_label_list, pop_num):
    ...
    for i in range(len(pop_score_list)):
        ...
        com.append([score, mlp_score, shap])
...
def recover_graph(original_graph_path, shap_value, failed_graph_dir):
    ...
    for failed_path in failed_graphs:
        ...
        all_benign_distance, all_benign_indices = benign_knn_model.kneighbors(feature)
        all_malware_distance, all_malware_indices = malware_knn_model.kneighbors(feature)
        score = all_benign_distance[0][0] - all_malware_distance[0][0]
...
def load_benign_and_malware_knn_model():
    benign_path = './430features_6yearsdataset_1000samples/apigraph_benignknn_10.pkl'
    malware_path = './430features_6yearsdataset_1000samples/apigraph_malwareknn_10.pkl'
...
if __name__ == '__main__':
    ...
    # for cal shap value
    MLP_model = load_MLP_model('./430features_6yearsdataset_1000samples/apigraphMLP_knn_1.h5')
    MLP_model2 = load_model('./430features_6yearsdataset_1000samples/apigraphMLP_knn_1.h5')
    ...
    # train_dataset = random.sample(first_column_6000, 100)
    # train_dataset = np.array(train_dataset)
    # print("train_dataset", train_dataset.shape)
    # 随机选择100个

    # load model, global variable
    model = load_model('/data/b/guoqi/icse25/malwareGA/model/apigraph/apigraphknn_1.pkl')
    ...
    # shap_dir = f'{save_dir}120_samples_knn_1_apigraph.pkl'
    ...
    for i in range(start_index, end_index):
        ...
        # fcg = recover_graph(fcg_file, zeros_list, recover_dir + 'ga_failed/')
        ...
        if non_zero_feature != 0:
            ...
            all_benign_distance, all_benign_indices = benign_knn_model.kneighbors(new_feature)
            all_malware_distance, all_malware_indices = malware_knn_model.kneighbors(new_feature)
            benign_distance = all_benign_distance[0][0]
            malware_distance = all_malware_distance[0][0]
            score = benign_distance - malware_distance
            MLP_Y_probs = MLP_model.predict(new_feature)
            MLP_Y_probs2 = MLP_model2.predict(new_feature)

            print(fcg.apk_name)
            print("score", score)
            print("MLP_Y_probs", MLP_Y_probs)
            print("MLP_Y_probs2", MLP_Y_probs2)
            # print("label", model.predict(new_feature))
            ...
            if score <= 0.0:
                ...
                print("malware")
                    # print(fcg.apk_name)
                ga(fcg, 40, 100, 300, False, None, False, True, False)
    ...
    # with open(f'{save_dir}120_samples_knn_1_apigraph.pkl', 'wb') as file:
