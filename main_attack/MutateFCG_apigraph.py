...
# load model
model = None
# save_dir = './mamadroid_test_only_self_defined/MLP/'  # 最后save的文件夹
save_dir = '/data/c/shiwensong/project/apigraph_test_flowdroid/MLP/'  # 最后save的文件夹
recover_dir = './mamadroid_test_new_mutation_with_shap/MLP/'  # 最后save的文件夹
shap_path = ''
init_num = 100
feature_dir = '/data/b/shiwensong/dataset/feature_Nov30/'
flowdroid_dir = '/data/b/shiwensong/dataset/120_samples_constraints/'
...
def ga(fcg, max_generation=100, pop_num=100, steps=500, init_state=False, population=None, sensitive=True, group = False):
...
def _population_score(fcg, pop, save_state=False):
    ...
    all_pop_score = test_MLP_model(pop_feature_list)
...
def load_MLP_model(model_path):
    ...
    MLP2 = Sequential([
        Dense(128, activation='relu', input_shape=(2704,)),  # 同样的输入维度
...
def recover_graph(original_graph_path, shap_value, failed_graph_dir):
    ...
    original_fcg = FCG_mamadroid(original_graph_path, 1, shap_value)
    ...
    mamadroid_matrix = np.zeros((11, 11))
    self_defined_nodes = set()
    obfuscated_nodes = set()
    system_nodes = set()

    families = list(mamadroid_feature_dict.keys())
    for failed_path in failed_graphs:
        ...
        #获取原图的点
        ori_nodes = list(original_fcg.original_call_graph.nodes._nodes.keys())
        for i in range(len(ori_nodes)):
            cur_node = ori_nodes[i]
            if i in nodes:
                family = current_fcg.smail_to_abstract(cur_node, families)
                if family == 'self-defined':
                    self_defined_nodes.add(i)
                elif family == 'obfuscated':
                    obfuscated_nodes.add(i)
                else:
                    system_nodes.add(i)
                mamadroid_feature_dict[family].add(i)

        for node in nodes:
            if node > 30000:
                self_defined_nodes.add(node)
                mamadroid_feature_dict['self-defined'].add(node)
            elif node > 50000:
                obfuscated_nodes.add(node)
                mamadroid_feature_dict['obfuscated'].add(node)

        for edge in edges:
            if edge[0] in self_defined_nodes and edge[1] in self_defined_nodes:
                mamadroid_matrix[9][9] += 1
            elif edge[0] in obfuscated_nodes and edge[1] in obfuscated_nodes:
                mamadroid_matrix[10][10] += 1
            elif edge[0] in self_defined_nodes and edge[1] in obfuscated_nodes:
                mamadroid_matrix[9][10] += 1
            elif edge[0] in obfuscated_nodes and edge[1] in self_defined_nodes:
                mamadroid_matrix[10][9] += 1
            else:
                family1 = node_to_type(edge[0], mamadroid_feature_dict)
                family2 = node_to_type(edge[1], mamadroid_feature_dict)
                mamadroid_matrix[families.index(family1)][families.index(family2)] += 1

        # print("mamadroid_matrix", mamadroid_matrix)

        current_fcg.mamadroid_matrix = mamadroid_matrix
        current_fcg.self_defined_nodes = self_defined_nodes
        current_fcg.obfuscated_nodes = obfuscated_nodes
        current_fcg.system_nodes = system_nodes
        current_fcg.mamadroid_feature_dict = mamadroid_feature_dict

        feature = current_fcg.cal_mamadroid_feature()
        feature = feature.flatten().reshape(1, -1)

        Y_probs = test_MLP_model(feature)
...
def node_to_type(node, mamadroid_feature_dict):
    for key, value in mamadroid_feature_dict.items():
        if node in value:
            return key
    return None
...
def recover_success_graph(original_graph_path, success_graph_path, shap_value):
    sample_name = original_graph_path.split("/")[-1].split(".")[0]
    print("sample_name", sample_name)

    # 1. load original graph
    original_fcg = FCG_apigraph(original_graph_path, 1, shap_value)

    # 2. load success graph
    with open('/data/b/guoqi/icse25/malwareGA/task/entity_class50.txt', 'r') as file:
        entity_class  = file.readlines()

    mamadroid_feature_dict = {i: set() for i in range(52)}
    mamadroid_matrix = np.zeros((52, 52))
    ...
    nodes, edges = load_graph(success_graph_path)
    ...
    for i in range(len(ori_nodes)):
        ...
        if i in nodes:
            family = current_fcg.smail_to_abstract(cur_node, families)
            if family == 50:#'self-defined'
                self_defined_nodes.add(i)
            elif family == 51:#'obfuscated'
                obfuscated_nodes.add(i)
            else:
                system_nodes.add(i)
            mamadroid_feature_dict[family].add(i)
    ...
    for node in nodes:
        if node > 30000 and node < 50000:
            self_defined_nodes.add(node)
            mamadroid_feature_dict[50].add(node)#'self-defined'
        elif node > 50000:
            obfuscated_nodes.add(node)
            mamadroid_feature_dict[51].add(node)#'obfuscated'

    for edge in edges:
        if edge[0] in self_defined_nodes and edge[1] in self_defined_nodes:
            mamadroid_matrix[50][50] += 1
        elif edge[0] in obfuscated_nodes and edge[1] in obfuscated_nodes:
            mamadroid_matrix[51][51] += 1
        elif edge[0] in self_defined_nodes and edge[1] in obfuscated_nodes:
            mamadroid_matrix[50][51] += 1
        elif edge[0] in obfuscated_nodes and edge[1] in self_defined_nodes:
            mamadroid_matrix[51][50] += 1
        else:
            family1 = node_to_type(edge[0], mamadroid_feature_dict)
            family2 = node_to_type(edge[1], mamadroid_feature_dict)
            if family1 is not None and family2 is not None:
                mamadroid_matrix[families.index(family1)][families.index(family2)] += 1
            else:
                print("error family1", family1)
                print("error family2", family2)
    ...
    feature = current_fcg.cal_mamadroid_feature()
    new_combined_feature = feature.flatten().reshape(1, -1)

    # Y_probs = test_MLP_model(new_combined_feature)
    # print("Y_probs", Y_probs[0][0])

    return feature.flatten()


def find_ori_graph_path(file_name):
    feature_dir = '/data/b/shiwensong/dataset/feature_Nov30/'
...
def obtain_new_trainset_single_feature(file_path):
    # 1. obtain all success files
    individual_file_names = glob.glob(file_path + "/*for_every_individual.json")
    # generation_file_names = glob.glob(file_path + "/*for_every_generation.json")
    print("individual_file_names", len(individual_file_names))
    # print("population_file_names", len(generation_file_names))
    individual = set()
    generation = set()
    ...
    for file_name in individual:
        success_file_path = file_path + "/ga_success/" + file_name + "_mutation_-1.gexf"
        ori_path = find_ori_graph_path(file_name)
        sample_feature = recover_success_graph(ori_path, success_file_path, None)
        all_features.append(sample_feature)
        if os.path.exists(success_file_path):
            success_files.append(success_file_path)
        else:
            print("error success_file_path", success_file_path)

    # save
    all_features = np.array(all_features)
    print("all_features", all_features.shape)
    train_dataset_dir = f'adversarial_apigraph_MLP_withoutshap.csv'
    train_dataset = pd.DataFrame(all_features)
    train_dataset.to_csv(train_dataset_dir, index=False)
    print("apigraph done!")
...
if __name__ == '__main__':
    ...
    #     tmp = tmp[:20]
    #     name.extend(tmp)
    name = glob.glob(flowdroid_dir + '*.gexf')
    ...
    # for cal shap value
    MLP_model = load_model('/data/b/guoqi/icse25/malwareGA/model/apigraph/apigraphMLP.h5')
    ...
    # # obtain the first
    # first_column_6000 = [row[0] for row in train_dataset[:6000]]
    # #random 100
    # first_column_6000 = random.sample(first_column_6000, 100)
    #
    # first_column_12000 = [row[0] for row in train_dataset[6000:12000]]
    # # random 100
    # first_column_12000 = random.sample(first_column_12000, 100)
    #
    # first_column_6000.extend(first_column_12000)
    #
    # first_column_6000 = np.array(first_column_6000)
    # print("train_dataset", len(first_column_6000))
    # 随机选择100个

    # train_dataset = first_column_6000

    # load model, global variable
    model = load_MLP_model('/data/b/guoqi/icse25/malwareGA/model/apigraph/apigraphMLP.h5')

    # obtain_new_trainset_single_feature(f'/data/b/shiwensong/project/malwareGA/task/apigraph_test_new_mutation_withoutsensitive/MLP/')
    # print("done")
    # sys.exit()
    ...
    # shap_dir = f'{save_dir}120_samples_MLP_apigraph.pkl'
    ...
    for i in range(start_index, end_index):
        ...
        if non_zero_feature != 0:
            ...
            Y_probs = test_MLP_model(new_feature)
            ...
            if Y_probs[0][0] < 0:
                ...
                print("malware")
                    # print(fcg.apk_name)
                ga(fcg, 40, 100, 300, False, None, False, False)
    ...
    # with open(f'{save_dir}120_samples_MLP_apigraph.pkl', 'wb') as file:
