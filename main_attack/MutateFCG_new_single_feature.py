...
# year = '2023'
save_dir = f'./{type}_testPL/MLP/'#最后save的文件夹
recover_dir = f'./{type}_test_mar22_second/MLP/'#从哪个文件夹里面恢复
...
def ga(fcg, max_generation = 100, pop_num = 100, steps = 500, init_state = False, population = None, sensitive = True, group = False):
    ...
    for i in range(max_generation):
        ...
        while True:
            ...
            length += len(new_individual.final_group_list)
            next_gene_population.append(new_individual)
        ...
        avg_length = length / pop_num
        with open(f'{fcg.apk_name}_before_solve_dependency_population_size.txt', 'a') as file:
...
def obtain_all_sample_name():
    individual_file_names = glob.glob(save_dir + "/*for_every_individual.json")
    generation_file_names = glob.glob(save_dir + "/*for_every_generation.json")
...
def obtain_new_trainset_single_feature(file_path):
    ...
    train_dataset_dir = f'adversarial_{type}_MLP_withoutshap.csv'
    train_dataset = pd.DataFrame(all_features)
    train_dataset.to_csv(train_dataset_dir, mode='a', index=False)
    print(type+"done!")
...
if __name__ == '__main__':
    ...
    # p.cpu_affinity(list(range(0, 30)))
    ...
    #for cal shap value
    MLP_model = load_model(f'./430features_6yearsdataset_1000samples/{type}MLP.h5')
    # print("MLP_model", MLP_model.summary())

    # train_dataset_dir = f'select_6years_train_dataset_for_shap_{feature_type}.csv'
    ...
    model = load_MLP_model(f'./430features_6yearsdataset_1000samples/{type}MLP.h5')

    obtain_new_trainset_single_feature(f'/data/b/shiwensong/project/malwareGA/task/{type}_test_withoutshap/MLP/')
    print("done")
    sys.exit()
    ...
    # shap_dir = f'{save_dir}120_samples_MLP_{feature_type}.pkl'
    ...
    # target_samples = ['a3b4f1a9799427cb715764ceccc0f429e148a3dce3f05d50bca0592bfcf233d7', 'VirusShare_57f49d9b916d158f7ce61ac338736583',
    #  'VirusShare_6372ad9ef6a5230bcec2a110d27736bd', '9a701b5c58d3aec46863be8e4bf411e1a4a143dd9ffd6062c608e2caf3a7a269',
    #  'd84ffbab7faa8aa63c8f98c52b66dffbd052efc72553b65b8073d30df1f3f39b']
    
    #withoutshap katz
    # target_samples = ['VirusShare_58bde35ee25f91c57ed7d3509603d824', '47844433ee4c2fbd4fd0dc0880bfbb23cfee7b5baa21146b172d53a63cbe4be0', 'VirusShare_37ca1c7a8f75b0cc52b93fbeb0e63026', ]

    #withoutshap degree
    # target_samples = ['01411844d63ecc8ef3a03069d314ee6f3d4064e7a40cfa9afa90ea74e3e96182', '47844433ee4c2fbd4fd0dc0880bfbb23cfee7b5baa21146b172d53a63cbe4be0', 'b4c7e0be7b01a784bf4f7208ed15e097a33e47f650775185488be534acf1549a', 'f3a5d605d3fefc29e6b9a83aa4482d6af23d6dda1171ca01457547a282f4f1e6']


    for i in range(start_index,end_index):
        fcg_file = name[i].replace('\n','')
        # target_name = fcg_file.split('/')[-1].split('.gexf')[0]
        # if target_name in target_samples:
        #     continue
        ...
        if fcg.used_sensitive_nodes is not None and len(fcg.used_sensitive_nodes) != 0:
            ...
            if Y_probs[0][0] < 0:
                ...
                print("malware")
            #     # print(fcg.apk_name)
                ga(fcg, 40, 100, 300, False, None, True, True)
            #     pop = _init_population(fcg, 5, 300)s
    ...
    # with open(f'{save_dir}120_samples_MLP_{feature_type}.pkl', 'wb') as file:
