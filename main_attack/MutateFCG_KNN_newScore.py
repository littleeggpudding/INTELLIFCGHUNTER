...
from Classify import train_model, test_model,get_distance_from_knn1, get_distance_from_knn5, get_4features_data, get_distance_from_original_knn1, get_index_from_knn1_benign_malware
import copy
import json
import psutil
import glob
...
model = None
MLP_model = None
parent_dir = 'debug'#最后save的文件夹
recover_path = 'feb7_KNN1_test_ga_MLP_score'#从哪个文件夹里面恢复
shap_path ='shap_values_120_samples_jan30_MLP.pkl'
init_num = 2
benign_neighbour_list = set()
malware_neighbour_list = set()
...
def ga(fcg, max_generation = 100, pop_num = 100, steps = 500, init_state = False, population = None):
    ...
    if not init_state:
        ...
        pop_score_list, mlp_pop_list, pop_shap_list = _population_score(fcg, population, False, True)
    ...
    for i in range(max_generation):
        ...
        print("check neighbor", len(benign_neighbour_list))
        print("check neighbor", len(malware_neighbour_list))
...
def _individual_score(fcg, individual, save_state = False, cal_neg = False):
    fcg = copy.deepcopy(fcg)
    ...
    # print("score", score)
    if cal_neg:
        benign_index, malware_index = get_index_from_knn1_benign_malware(combined_feature)
        malware_neighbour_list.add(malware_index)
        benign_neighbour_list.add(benign_index)
...
def _population_score(fcg, pop, save_state = False, cal_neg = False):
    ...
    for i in range(len(pop) - 1, -1, -1):
        ...
        score, mlp_score, shap_sum = _individual_score(fcg, pop[i], save_state, cal_neg)
...
if __name__ == '__main__':
    ...
    #KNN second ga
    # target_sample_slices = [0, 2, 4, 6, 13, 15, 16, 17, 18, 19, 23, 24, 28, 32, 33, 35, 36, 39, 42, 46, 49, 50, 54, 55, 57, 62, 64, 65, 70, 72, 75, 76, 77, 79, 82, 85, 86, 90, 92, 93, 108, 110, 115, 119]
    ...
    #load shap value
    with open('shap_values_120_samples_feb27_KNN1.pkl', 'rb') as file:
    ...
    MLP_model = load_MLP_model('./430features_3yearsdataset_all/MLP.h5')
    ...
    # with open('shap_values_31_samples_feb27_recover_dominate_shap_score_init.pkl', 'wb') as file:
