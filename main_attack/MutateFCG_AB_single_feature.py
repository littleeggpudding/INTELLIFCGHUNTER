...
model_name = '100Estimator_AdaBoost'
benign_knn_model = None
malware_knn_model = None
feature_type = 'degree'
save_dir = f'/data/c/shiwensong/project/test_mlp_substitute/3Layers_RandomForest/{feature_type}/'#最后save的文件夹
...
def _population_score(fcg, pop, save_state = False, sub = True):
    ...
    tree_shap = get_shap_other_tree_explain()
...
def visit_random_forest(sample, model, can_modify):
    ...
    for tree in model.estimators_:
        ...
        find_alternative_decision_points(cur_path, all_paths, can_modify)
        i += 1
    ...
    #         continue
        ...
        if fcg.used_sensitive_nodes is not None and len(fcg.used_sensitive_nodes) != 0:
            ...
            if score[0][1] < 0.5:
                ...
                print("malware")
                print(fcg.apk_name)
                # ga(fcg, 40, 5, 800, False, None, True, True, False)
