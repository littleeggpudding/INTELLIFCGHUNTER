...
model = None
MLP_model = None
save_dir = './all_test_mar22/RandomForest_new'  # 最后save的文件夹
...
def ga(fcg, max_generation=100, pop_num=100, steps=500, init_state=False, population=None):
    ...
    if not init_state:
        ...
        if init_num > pop_num:
            indexes = np.argsort(pop_tree_shap_list)[::-1][:pop_num]
...
def select(pop_score_list, pop_mlp_list, pop_shap_list, pop_tree_shap_list, idxes=None):
    ...
    for i in range(len(pop_score_candidate)):
        ...
        score = pop_score_candidate[i] * 100 #调整权重
        shap = -pop_shap_candidate[i] #干扰项
        tree_shap = -pop_tree_shap_candidate[i] * 100 #调整权重
...
def sort_all_pop(pop, pop_score_list, pop_mlp_list, pop_shap_list, pop_tree_shap_list, pop_num):
    ...
    for i in range(len(pop_score_list)):
        ...
        score = pop_score_list[i] * 100 #调整权重
        shap = -pop_shap_list[i] #干扰项
        tree_shap = -pop_tree_shap_list[i] * 100 #调整权重
...
def recover_graph(sample_name, shap_value,
    ...
    for failed_path in failed_graphs:
        ...
        failed_graph_scores.append((MLP_Y_probs[0][0], current_fcg))
...
if __name__ == '__main__':
    ...
    txt_file = f'/data/b/guoqi/malware/tree_attack_set/baseline_AdaBoost_combined.txt'
    with open(txt_file, 'r') as file:
        tmp = file.readlines()
        name.extend(tmp)
    ...
    # for cal shap value
    MLP_model = load_MLP_model(f'./430features_6yearsdataset_1000samples/allMLP_{model_name}.h5')
    MLP_model2 = load_model(f'./430features_6yearsdataset_1000samples/allMLP_{model_name}.h5')
    ...
    model = load_model(f'./430features_6yearsdataset_1000samples/all3Layers_RandomForest.pkl')
    ...
    # #load substitute shap
    # shap_dir = f'{save_dir}120_samples_{model_name}.pkl'
    # with open(shap_dir, 'rb') as file:
    #     new_shap_values = pickle.load(file)
    # new_shap_values = new_shap_values[0]

    #load tree shap
    # tree_shap_dir = f'{save_dir}120_samples_{model_name}_tree.pkl'
    # with open(tree_shap_dir, 'rb') as file:
    #     new_tree_shap_values = pickle.load(file)
    # new_tree_shap_values = new_tree_shap_values[1] #对于类别1
    # print("new_tree_shap_values", new_tree_shap_values.shape)
    ...
    for i in range(start_index, end_index):
        ...
        fcg = FCG(fcg_file, 1, zeros_list, zeros_list)
        # fcg = FCG(fcg_file, 1)
        ...
        if fcg.used_sensitive_nodes is not None and len(fcg.used_sensitive_nodes) != 0:
            ...
            if Y_probs[0][1] < 0.5:
                ...
                # score_list, mlp_score_list, shap_list, tree_shap_list, pop_feature_list = _population_score(fcg, pop)
                ...
                # else:
                #     # save pop_feature_list
                #     all_pop_feature.extend(pop_feature_list)
                #     print("all_pop_feature shape", len(all_pop_feature))
                # break


    all_features = np.array(all_samples_features)
    # new_shap_values = get_shap(train_dataset, all_features, MLP_model2)
    # with open(f'{save_dir}120_samples_{model_name}.pkl', 'wb') as file:
    ...
    with open(f'{save_dir}120_samples_{model_name}_tree.pkl', 'wb') as file:
        pickle.dump(new_shap_values, file)
