...
#load model
model = None
save_dir = '/data/c/shiwensong/project/RF_test_no_pca_new/'#最后save的文件夹
...
def _population_score(fcg, pop, save_state = False):
    ...
    original_combined_feature = None
    ...
    # new_pop_feature_list = pca.transform(pop_feature_list)
    new_pop_feature_list = pop_feature_list
    all_pop_score = model.predict_proba(new_pop_feature_list)
    shap_list = get_shap_other_tree_explain(new_pop_feature_list,model)
    shap_list = shap_list[1]
    for i in range(len(all_pop_score)):
        score = all_pop_score[i][1]
        score_list.append(score)

        shap_value = shap_list[i]
        combined_feature = pop_feature_list[i]
        shap_sum = calculate_opposite_adjustment(original_combined_feature, combined_feature, shap_value)

        shap_score_list.append(shap_sum)
    ...
    # 返回修改后的 score_list
    return score_list[::-1], shap_score_list[::-1]
...
if __name__ == '__main__':
    ...
    model = load_model('/data/b/shiwensong/project/malwareGA/task/allfeatures_6yeardataset_1000samples/all3Layers_RandomForest.pkl')
    ...
    for i in range(start_index,end_index):
        ...
        if fcg.used_sensitive_nodes is not None and len(fcg.used_sensitive_nodes) != 0:
            ...
            combined_feature = np.hstack((degree, katz, closeness, harmonic))
            combined_feature = combined_feature.reshape(1, -1)
            # new_feature = pca.transform(combined_feature)
            end_time = time.time()
            ...
            new_feature = combined_feature
            Y_probs = model.predict_proba(new_feature)
            # Y_probs = test_MLP_model(combined_feature)
            end_time = time.time()
            ...
            if Y_probs[0][1] < 0.5:
                ...
                print("malware")
            #     # print(fcg.apk_name)
                ga(fcg, 40, 5, 300, False, None, True, False)
