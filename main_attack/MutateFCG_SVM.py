...
#load model
model = None
save_dir = './all_test_mar22/SVM/'#最后save的文件夹
recover_dir = './all_test_mar22/SVM/'#最后save的文件夹
shap_path =''
init_num = 100
...
def _population_score(fcg, pop, save_state = False):
    ...
    all_pop_score =  model.predict_proba(pop_feature_list)
    all_shap = model.decision_function(pop_feature_list)
...
if __name__ == '__main__':
    ...
    model = load_model('./430features_6yearsdataset_1000samples/allSVM.pkl')
    ...
    for i in range(start_index,end_index):
        ...
        if fcg.used_sensitive_nodes is not None and len(fcg.used_sensitive_nodes) != 0:
            ...
            decision = model.decision_function(combined_feature)
            proba = model.predict_proba(combined_feature)
            # Y_probs = test_MLP_model(combined_feature)
            end_time = time.time()
    ...
    # new_shap_values = get_shap(train_dataset, all_features, model)
    # with open(f'{save_dir}120_samples_SVM.pkl', 'wb') as file:
