...
from Classify import train_model, test_model,get_distance_from_knn1, get_distance_from_knn5, get_4features_data, get_distance_from_original_knn1, finetune_model
import copy
import json
import psutil
import glob
...
recover_path = 'test_shap'#从哪个文件夹里面恢复
shap_path =''
init_num = 100
...
def ga(fcg, max_generation = 100, pop_num = 100, steps = 500, init_state = False, population = None):
    ...
    for i in range(max_generation):
        ...
        while True:
            ...
            individual1_index, individual2_index = _ga_select(population, mlp_pop_list, pop_shap_list, int(pop_num/2)-1)
...
def _individual_score(fcg, individual, save_state = False):
    ...
    return Y_probs[0], MLP_Y_probs[0][0], shap_sum
...
def recover_graph(sample_name, shap_value, failed_graph_dir = 'log_init_200pop_100step_shap/feb7_MLP_test_ga_dominate_shap_score_init/ga_failed/', k = 0):
    ...
    print("original_fcg_file", original_fcg_file)
    ...
    for failed_path in failed_graphs:
        ...
        Y_probs = model.predict(combined_feature)
        MLP_Y_probs = MLP_model.predict(combined_feature)
        print("KNN probs", Y_probs[0])
        print("MLP probs", MLP_Y_probs[0][0])
...
def recover_failed_graph(sample_name, shap_value, failed_graph_dir = 'log_init_200pop_100step_shap/feb7_MLP_test_ga_dominate_shap_score_init/ga_failed/', k = 0):
    success_graph = f'{recover_path}/{year}/ga_success/{sample_name}_mutation_-1.gexf'
    if os.path.exists(success_graph):
        print("success_graph", success_graph)
        return None
    ...
    for failed_path in failed_graphs:
        ...
        harmonic = current_fcg.harmonic_feature
        combined_feature = np.hstack((degree, katz, closeness, harmonic))
        failed_features.append(combined_feature)
    ...
    print("failed_features", len(failed_features))

    return failed_features

def recover_succcess_graph(sample_name, shap_value):
    # 1. find success graphs
    success_graph = f'{recover_path}/{year}/ga_success/{sample_name}_mutation_-1.gexf'
    if not os.path.exists(success_graph):
        return None

    print("success_graph", success_graph)

    #if it is a success graph, then we need to recover it
    print("recover_graph", sample_name)
    ...
    # 3. recover a sucess graph
    nodes, edges = load_graph(success_graph)
...
if __name__ == '__main__':
    ...
    # for cal shap value
    MLP_model = load_MLP_model('./430features_6yearsdataset_all/MLP_knn_1_retrained.h5')
    ...
    #using all success samples to train the model
    for year in ['2022', '2023']:
        ...
        for i in range(len(name)):
            ...
            # fcg = FCG(fcg_file, 0, zeros_list)
            load_start = time.time()
            sample_name = fcg_file.split('/')[-1].split('.')[0]
            failed_graph_dir = f'{recover_path}/{year}/ga_failed/'

            features = recover_failed_graph(sample_name, zeros_list, failed_graph_dir, 0)
            if features is None or len(features) == 0:
                continue

            # feature = recover_succcess_graph(sample_name, zeros_list)
            # if feature is None:
            #     continue

            all_samples_features.extend(features)

            print("during cal all_samples_features", len(all_samples_features))

    print("all_samples_features", len(all_samples_features))
    all_features = np.array(all_samples_features)
    print("all_samples_features", all_features.shape)
    #generate label
    labels = [0] * len(all_samples_features)
    labels = np.array(labels)
    path = finetune_model(MLP_model, all_features, labels, './430features_6yearsdataset_all/')
    print("path", path)
