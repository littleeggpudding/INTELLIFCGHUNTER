...
# load model
model = None
save_dir = '/data/c/shiwensong/project/baseline_mamadroid/ab_random/'  # 最后save的文件夹
recover_dir = '/data/c/shiwensong/project/baseline_mamadroid/ab_random/'  # 最后save的文件夹
shap_path = ''
init_num = 100
...
def ga(fcg, max_generation=100, pop_num=100, steps=500, init_state=False, population=None, sensitive=True):
    ...
    for i in range(max_generation):
        ...
        while True:
            ...
            # 2.6 ga mutation
            new_individual.ga_mutation_add_edge(cur_fcg, 0.2, sensitive)
...
def obtain_all_failed_sample_name():
    ...
    return failed, union, individual
...
if __name__ == '__main__':
    ...
    #     tmp = tmp[:20]
    #     name.extend(tmp)
    with open ('/data/b/guoqi/malware/tree_attack_set/selected_AdaBoost_mamadroid.txt', 'r') as file:
        name = file.readlines()
    ...
    all_sample_name, union, individual = obtain_all_failed_sample_name()
    ...
    # for cal shap value
    # sub_model = load_GCN_model('/data/b/shiwensong/project/malwareGA/task/gcn_model.pth')
    ...
    target_model = load_model('./430features_6yearsdataset_1000samples/family100Estimator_AdaBoost.pkl')
    ...
    for i in range(start_index, end_index):
        ...
        if non_zero_feature != 0:
            ...
            if Y_probs[0][1] < 0.5:
                ...
                pop = _init_population(fcg, 100, 300)
                pop_score_list, pop_shap_list = _population_score(fcg, pop)
                target_index = _fitness_function(pop_score_list)
                if target_index != -1:
                    print("fitness function is true")
                    save_log_for_every_individual(fcg, pop[target_index], pop_score_list[target_index], i)
