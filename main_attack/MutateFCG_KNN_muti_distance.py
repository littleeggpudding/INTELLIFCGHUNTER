...
from Classify import train_model, test_model,get_distance_from_knn1, get_distance_from_knn5, get_4features_data, get_distance_from_original_knn1, get_distance_from_knn10
import copy
import json
import psutil
import glob
...
model = None
MLP_model = None
save_dir = 'test_shap_v2_new_second_distance'#最后save的文件夹
feature_dir = '/data/b/shiwensong/dataset/feature_Nov30/'
recover_path = 'test_shap_v2_new'#从哪个文件夹里面恢复
shap_path =''
init_num = 200
year = '2021'
original_graph_dir = ''
if year == '2023':
    original_graph_dir = f'/data/b/shiwensong/dataset/virusshare{year}_3_gexf/'
elif year == '2022':
    original_graph_dir = f'/data/b/shiwensong/dataset/virusshare{year}_2_gexf/'
else:
    original_graph_dir = f'/data/b/shiwensong/dataset/virusshare{year}_gexf/'
...
def ga(fcg, max_generation = 100, pop_num = 100, steps = 500, init_state = False, population = None):
    ...
    if not init_state:
        ...
        pop_score_list, mlp_pop_list, pop_shap_list = _population_score(fcg, population)
    ...
    for i in range(max_generation):
        ...
        if i == max_generation - 1:
            save_state = True
        cur_pop_score_list, cur_mlp_pop_list, cur_pop_shap_list = _population_score(fcg, next_gene_population, save_state)
...
def _individual_score(fcg, individual, save_state = False):
    ...
    benign_distances, benign_indices = benign_knn_model.kneighbors(combined_feature)
    benign_distance = benign_distances[0][0]
    
    malware_distances, malware_indices = malware_knn_model.kneighbors(combined_feature)
    malware_distance = malware_distances[0][0]
    ...
    # # else:#malware
    knn_score = benign_distance - malware_distance
    ...
    return knn_score, MLP_Y_probs[0][0], shap_sum
...
def save_log_for_every_individual(fcg, individual, score, generation):
    ...
    try:
        ...
        file_name = f'{save_dir}/{year}/{fcg.apk_name}_log_for_every_individual.json'
...
def save_graph_for_every_generation(fcg, state, generation):
    if state == 'failed':
        fcg.save(f'{save_dir}/{year}/ga_failed/', generation)
    elif state == 'success':
        fcg.save(f'{save_dir}/{year}/ga_success/', generation)
...
def save_log_for_every_generation(fcg, score, shap, generation, population):
    ...
    try:
        ...
        file_name = f'{save_dir}/{year}/{fcg.apk_name}_log_for_every_generation.json'
        with open(file_name, 'a') as file:
            file.write(json_string + '\n')
        # print("save mutation log successfully!")
    except Exception as e:
...
def _population_score(fcg, pop, save_state = False):
    ...
    for i in range(len(pop) - 1, -1, -1):
        ...
        score, mlp_score, shap_sum = _individual_score(fcg, pop[i], save_state)
...
def recover_graph(sample_name, shap_value, failed_graph_dir = 'log_init_200pop_100step_shap/feb7_MLP_test_ga_dominate_shap_score_init/ga_failed/'):
    success_graph = f'{recover_path}/{year}/ga_success/{sample_name}_mutation_-1.gexf'
    if os.path.exists(success_graph):
        print("success_graph", success_graph)
        return None

    #if it it not a success graph, then we need to recover it
    print("recover_graph", sample_name)

    # 1. load failed graphs, including all failed graphs
    ...
    for failed_path in failed_graphs:
        try:
            ...
            new_combined_feature = combined_feature.reshape(1, -1)

            Y_probs = model.predict(new_combined_feature)
            MLP_Y_probs = MLP_model.predict(new_combined_feature)

            if MLP_Y_probs[0][0] < min_score:
                min_score = MLP_Y_probs[0][0]
                min_failed_graph = current_fcg
                min_failed_graph_name = failed_path
    ...
    return min_failed_graph, combined_feature, min_failed_graph_name

def recover_top1_graph(sample_name, shap_value, failed_graph_file):
    if not os.path.exists(failed_graph_file):
        print("not recover_graph", failed_graph_file)
        return None
    ...
    # 3. cal the scores, and find the best k failed graph
    nodes, edges = load_graph(failed_graph_file)
    ...
    if Y_probs[0] == 1:
        ...
        min_failed_graph = current_fcg
        min_failed_graph = deal_with_graph(min_failed_graph)
        return min_failed_graph, combined_feature
    else:
        return None
...
def deal_with_graph(min_failed_graph):
    ...
    # remove_edges = []
    # for edge in min_failed_graph.edges:
    #     if edge[0] not in min_failed_graph.nodes or edge[1] not in min_failed_graph.nodes:
    #         remove_edges.append(edge)
    #
    # for edge in remove_edges:
    #     min_failed_graph.edges.remove(edge)
    #
    # remove_nodes = []
    # for node in min_failed_graph.nodes:
    #     for edge in min_failed_graph.edges:
    ...
    #
    # for node in remove_nodes:
    #     min_failed_graph.nodes.remove(node)
    #     if node in min_failed_graph.user_defined_nodes:
    #         min_failed_graph.user_defined_nodes.remove(node)
...
def load_benign_and_malware_knn_model():
    benign_path = './430features_6yearsdataset_all/benignknn_10.pkl'
    malware_path = './430features_6yearsdataset_all/malwareknn_10.pkl'
...
if __name__ == '__main__':
    ...
    txt_file = f'{feature_dir}mar12_hard_attack_samples_{year}_{round}.txt'
    ...
    # failed_graph_file = f'{recover_path}/{year}/best_failed_graph.txt'
    # with open(failed_graph_file, 'r') as file:
    #     failed_graphs = file.readlines()
    #
    # name = failed_graphs
    # print("failed_graph", len(failed_graphs))
    # print("failed_graph 0 ", failed_graphs[0])
    ...
    # for cal shap value
    MLP_model = load_MLP_model('./430features_6yearsdataset_all/MLP_knn_1.h5')
    MLP_model2 = load_model('./430features_6yearsdataset_all/MLP_knn_1.h5')
    ...
    model = load_model('./430features_6yearsdataset_all/knn_1.pkl')
    ...
    # with open(f'test_{year}_{round}_new.pkl', 'rb') as file:
    ...
    for i in range(start_index, end_index):
        ...
        failed_graph_dir = f'{recover_path}/{year}/ga_failed/'
        # fcg = recover_graph(sample_name, new_shap_values[i], failed_graph_dir, 0)
        res = recover_graph(sample_name, zeros_list, failed_graph_dir)
        print("sample_name", sample_name)
        # fcg, combined_feature = recover_top1_graph(sample_name, new_shap_values[i], failed_graph)
        if res is None:
            continue

        fcg, combined_feature, file_name = res
        #
        # save_best_graph = f'{recover_path}/{year}/best_failed_graph.txt'
        # with open(save_best_graph, 'a') as file:
    ...
    print("all_samples_features", all_features.shape)
    new_shap_values = get_shap(train_dataset, all_features, MLP_model2)
    with open(f'test_{year}_{round}_new.pkl', 'wb') as file:
