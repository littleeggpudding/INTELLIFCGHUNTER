...
if __name__ == '__main__':
    ...
    # txt_file = f'{feature_dir}/march9_new_attack_samples_{year}.txt'
    ...
    model1 = load_model('./430features_6yearsdataset_all/MLP.h5')
    model2 = load_model('./430features_6yearsdataset_all/knn_1.pkl')
    model3 = load_model('./430features_6yearsdataset_all/knn_3.pkl')
    model4 = load_model('./430features_6yearsdataset_all/RandomForest.pkl')
    model5 = load_model('./430features_6yearsdataset_all/AdaBoost.pkl')

    for year in years:
        csv_file = f'/data/b/shiwensong/dataset/feature_Nov30/degree_1_{year}.csv'
        if year == 2022:
            gexf_dir = f'/data/b/shiwensong/dataset/virusshare{year}_2_gexf/'
        elif year == 2023:
            gexf_dir = f'/data/b/shiwensong/dataset/virusshare{year}_3_gexf/'
        else:
            gexf_dir = f'/data/b/shiwensong/dataset/virusshare{year}_gexf/'
        gexfs = glob.glob(gexf_dir + '*.gexf')
        print("gexfs", len(gexfs))
        #read csv file
        df = pd.read_csv(csv_file)
        #get the first column
        first_column = df.iloc[:, 0]
        print(len(first_column.values))
        # extract nums[i] samples
        num = nums[years.index(year)]
        first_column = first_column[:num] #train set
        print("before first_column", len(first_column))
        first_column = set(first_column.values)
        print("first_column", len(first_column))
        fcg_list = set()
        node_edge = set()
        for j in range(len(gexfs)):
            cur_gexf = gexfs[j]
            sample_name = gexfs[j].split('/')[-1].split('.')[0]
            print(sample_name)
            if sample_name in first_column:
                continue

            try:
                # print("cur_gexf", cur_gexf)
                fcg = FCG(cur_gexf, 1, 0)
                # print("sample_name", cur_gexf)
                # print("fcg nodes", len(fcg.nodes))
                # print("fcg edges", len(fcg.edges))
                ...
                cnt = 0
                if fcg.used_sensitive_nodes is not None and len(fcg.used_sensitive_nodes) != 0 and (len(fcg.nodes) < 30000 or len(fcg.edges) < 30000):
                    ...
                    y_predit1 = model1.predict(new_combined_feature)
                    y_predit2 = model2.predict(new_combined_feature)
                    y_predit3 = model3.predict(new_combined_feature)
                    y_predit4 = model4.predict(new_combined_feature)
                    y_predit5 = model5.predict(new_combined_feature)
                    # print("y_predit1", y_predit1)
                    # print("y_predit2", y_predit2)
                    # print("y_predit3", y_predit3)
                    # print("y_predit4", y_predit4)
                    # print("y_predit5", y_predit5)
                    end_time = time.time()
                    print("test model time", end_time - begin_time)

                    if y_predit1[0][0] < 0.5 and y_predit2[0] == 0 and y_predit3[0] == 0 and y_predit4[0] == 0 and y_predit5[0] == 0:
                        print("benign")
                        # write in txt
                        ...
                        print("malware")
                        with open(f'{feature_dir}new_attack_samples_features_{year}_.csv', 'a') as file:
