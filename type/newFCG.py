...
from ExtractFeature import obtain_sensitive_apis, extract_degree_centrality_from_CG, \
    extract_katz_centrality_from_CG, extract_harmonic_centrality_from_CG, extract_closeness_centrality_from_CG, extract_closeness_harmonic_centrality_from_CG, extract_harmonic_centrality_from_CG_new, extract_closeness_centrality_from_CG_new
import os
...
class FCG:
    ...
    def __init__(self, file_path, label, shap_value=None, gcn=False):
    ...
    def process_mutation(self, mutation):
        ...
        if type == 'remove_node':
            ...
            if add_node in self.nodes:
                ...
                return state, res
        elif type == 'directlink':
            # 不可能冲突，直接连就行
            self.edges.add(tuple(add_edges[0]))
            return state, res
        elif type == 'relay':
            # 不可能冲突，把两点直接原本的边删掉，然后加上若干新的点和新的边
            self.edges.remove(tuple(remove_edges[0]))
            for add_node in add_nodes:
                self.nodes.add(add_node)
            for add_edge in add_edges:
                self.edges.add(tuple(add_edge))
        elif type == 'add_many_nodes':
            # 不可能冲突，直接加就行
            for add_node in add_nodes:
                self.nodes.add(add_node)
        return state, res
    def process_mutation_no_conflict(self, mutation):
        mutation = mutation.mutation
        ...
        if type == 'directlink':
            ...
            for add_node in add_nodes:
                self.nodes.add(add_node)
        return True
    # def process_mutation_sequence_no_conflict(self, mutations):  # mutations是一个list，里面存储的是Mutation
    #     final_state = True
    #     for mutation in mutations:
    #         state = self.process_mutation_no_conflict(mutation)
    #     return final_state
    ...
    def build_mutation_relay(self, startnode, endnode, number=1):
        # 修改敏感边，添加若干个节点
        mutation = Mutation(self.apk_name)
        tmp_startnode = startnode
        for i in range(number):
            new_node = FCG.number + 40000
            FCG.number = FCG.number + 1
            mutation.mutation['add_nodes'].append(new_node)
            mutation.mutation['add_edges'].append((tmp_startnode, new_node))
            tmp_startnode = new_node
        mutation.mutation['add_edges'].append((tmp_startnode, endnode))
        mutation.mutation['remove_edges'].append((startnode, endnode))
        mutation.mutation['feature_type'] = 'relay'
        return mutation
    def build_mutation_directlink(self, startnode, endnode):
        # 添加直连边
        mutation = Mutation(self.apk_name)
        mutation.mutation['add_edges'].append((startnode, endnode))
        mutation.mutation['feature_type'] = 'directlink'
        return mutation
    def build_mutation_add_many_nodes(self, number=1):
        mutation = Mutation(self.apk_name)
        ...
        mutation.mutation['feature_type'] = 'add_many_nodes'
        return mutation
