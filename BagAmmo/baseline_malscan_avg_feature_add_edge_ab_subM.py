...
model = None
feature_type = 'avg'
save_dir = f'/data/c/shiwensong/project/baseline_malscan_{feature_type}_M/ab'  # 最后save的文件夹
recover_dir = f'/data/c/shiwensong/project/baseline_malscan_{feature_type}_M/ab'  # 最后save的文件夹
shap_path = ''
init_num = 100
...
if __name__ == '__main__':
    ...
    #     tmp = tmp[:20]
    #     name.extend(tmp)
    with open ('/data/b/guoqi/malware/tree_attack_set/selected_AdaBoost_average.txt', 'r') as file:
        name = file.readlines()
    ...
    all_sample_name, union, success = obtain_all_failed_sample_name()

    failed_sample = []
    for i in range(len(name)):
        # obtain apk_name
        apk_name = name[i].split('/')[-1].split('.gexf')[0]
        if apk_name not in success:
    ...
    # for cal shap value
    sub_model = load_MLP_model(f'./430features_6yearsdataset_1000samples/{feature_type}MLP_AdaBoost.h5')
    ...
    target_model = load_model(f'./430features_6yearsdataset_1000samples/{feature_type}100Estimator_AdaBoost.pkl')
