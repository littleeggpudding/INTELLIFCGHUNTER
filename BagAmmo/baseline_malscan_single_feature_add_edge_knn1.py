...
model = None
feature_type = 'degree'
# save_dir = f'/data/c/shiwensong/project/baseline_malscan/{feature_type}/knn_1/'  # 最后save的文件夹
save_dir = 'bagammo_degree_knn1'
recover_dir = f'/data/c/shiwensong/project/baseline_malscan/{feature_type}/knn_1/'  # 最后save的文件夹
shap_path = ''
init_num = 100
...
def ga(fcg, max_generation=100, pop_num=100, steps=500, init_state=False, population=None, sensitive=True):
    ...
    for i in range(max_generation):
        ...
        while True:
            ...
            with open('individual_time_bagammo.txt', 'a') as file:
                file.write(str(i) + " " + str(individual_end - individual_start) + '\n')
        ...
        with open('generation_time_bagammo.txt', 'a') as file:
...
def _init_population(fcg, pop_num=100, steps=500, sensitive=True):
    ...
    while try_times < init_num:
        ...
        mutationSequence = MutationSequence(mutation_lists, False)
...
if __name__ == '__main__':
    ...
    target_model = load_model(f'./430features_6yearsdataset_1000samples/{feature_type}knn_1.pkl')
    ...
    for i in range(start_index, end_index):
        ...
        if fcg.used_sensitive_nodes is not None and len(fcg.used_sensitive_nodes) != 0 and feature is True:
            ...
            new_feature = np.array(combined_feature).reshape(1, -1)
