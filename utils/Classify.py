...
def load_test_model(model_path = '/data/b/shiwensong/project/malwareGA/main_attack/430features_6yearsdataset_all/knn_1.pkl', shape = 2704):
    #vector和label都是numpy格式
    #model_path是一个文件夹
    # print(model_path)
...
def malware_dataset_apigraph():
    X_benign = []
    Y_benign = []
    for year in ['2018', '2019', '2020', '2021', '2022', '2023']:
        feature_dir = f'/data/b/shiwensong/dataset/feature_Nov30/apigraph_1_{year}.csv'
        X_train, Y_train = obtain_apigraph_feature(feature_dir)
        # 取80%
        length = int(len(Y_train) * 0.8)
        X_train = X_train[:length]
        Y_train = Y_train[:length]
        X_benign.extend(X_train)
...
def benign_dataset_apigraph():
    X_benign = []
    Y_benign = []
    for year in ['2018', '2019', '2020', '2021', '2022', '2023']:
        feature_dir = f'/data/b/shiwensong/dataset/feature_Nov30/apigraph_0_{year}.csv'
        X_train, Y_train = obtain_apigraph_feature(feature_dir)
        # 取80%
        length = int(len(Y_train) * 0.8)
        X_train = X_train[:length]
...
def test_malware_dataset_apigraph():
    X_benign = []
    Y_benign = []
    for year in ['2018', '2019', '2020', '2021', '2022', '2023']:
        feature_dir = f'/data/b/shiwensong/dataset/feature_Nov30/apigraph_1_{year}.csv'
...
def test_benign_dataset_apigraph():
    X_benign = []
    Y_benign = []
    for year in ['2018', '2019', '2020', '2021', '2022', '2023']:
        feature_dir = f'/data/b/shiwensong/dataset/feature_Nov30/apigraph_0_{year}.csv'
...
def test_benign_dataset_mamadroid_feature(type = 'family'):
    ...
    for year in ['2018', '2019', '2020', '2021', '2022', '2023']:
        ...
        X_train = X_train[length_80:]
        Y_train = Y_train[length_80:]
        X_benign = vstack([X_benign, X_train])
        Y_benign.extend(Y_train)
    print("X_benign", X_benign.shape)
    print("Y_train", len(Y_benign))
...
def obtain_apigraph_feature(file_path):
    # 读取CSV文件，每行作为字符串处理
    sha256 = set()
    X_train = []
    Y_train = []
    with open(file_path, 'r') as file:
        ...
        for line in lines:
            cur_sha256 = line.split(',')[0]
            if cur_sha256 in sha256:
                continue
            sha256.add(cur_sha256)
            cur_features = line.split(',')[1:-1]
            cur_label = line.split(',')[-1]
            cur_features = [float(x) for x in cur_features]
            X_train.append(cur_features)
            Y_train.append(int(cur_label))
            if len(sha256) >= 1000:
                break
...
if __name__ == '__main__':
    ...
    for feature_type in ['apigraph']:
        if feature_type == 'family':
            ...
            X_benign, Y_benign = benign_dataset_apigraph()
            X_malware, Y_malware = malware_dataset_apigraph()
        else:
        ...
        model_output_path = './430features_6yearsdataset_1000samples/'
        # rf = train_model(X_dataset, Y_dataset, 'RandomForest', model_output_path, feature_type)
        # ab = train_model(X_dataset, Y_dataset, 'AdaBoost', model_output_path, feature_type)
        # knn1 = train_model(X_dataset, Y_dataset, 'KNN_1', model_output_path, feature_type)
        # knn3 = train_model(X_dataset, Y_dataset, 'KNN_3', model_output_path, feature_type)
        mlp = train_model(X_dataset, Y_dataset, 'MLP', model_output_path, feature_type)
    #     #
        ...
        rf = f'{model_output_path}/{feature_type}3Layers_RandomForest.pkl'
        model = joblib.load(rf)
        print("model", model)
        cur_model_name = 'RandomForest'
        train_substitute_Model(X_dataset, model, 'MLP', cur_model_name, model_output_path, feature_type)
        ab = f'{model_output_path}/{feature_type}100Estimator_AdaBoost.pkl'
        model = joblib.load(ab)
        print("model", model)
        cur_model_name = 'AdaBoost'
        train_substitute_Model(X_dataset, model, 'MLP', cur_model_name, model_output_path, feature_type)
    ...
    #     shape = 430
        if feature_type == 'avg':
            ...
            X_benign, Y_benign = test_benign_dataset_apigraph()
            X_malware, Y_malware = test_malware_dataset_apigraph()
            shape = 2704
        else:
        ...
        # model1 = joblib.load(f'{model_dir}{feature_type}100Estimator_AdaBoost.pkl')
        # print("AdaBoost")  # 0 1
        # cal_and_test_model(X_dataset, Y_dataset, model1)
        # model2 = joblib.load(f'{model_dir}{feature_type}3Layers_RandomForest.pkl')
        # print("RandomForest")  # 0 1
        # cal_and_test_model(X_dataset, Y_dataset, model2)
        # #     print("knn_1") # 0 1
        # model3 = joblib.load(f'{model_dir}{feature_type}knn_1.pkl')
        # cal_and_test_model(X_dataset, Y_dataset, model3)
        # model4 = joblib.load(f'{model_dir}{feature_type}knn_3.pkl')
        # print("knn_3")  # 0 1
        # cal_and_test_model(X_dataset, Y_dataset, model4)

        model5 = load_test_model(f'{model_dir}{feature_type}MLP_RandomForest.h5', shape)
        print("rf_mlp")  # float[0] 1
        cal_and_test_model(X_dataset, Y_dataset, model5, '123')
        model5 = load_test_model(f'{model_dir}{feature_type}MLP_AdaBoost.h5', shape)
        print("ab_mlp")  # float[0] 1
        cal_and_test_model(X_dataset, Y_dataset, model5, '123')
    ...
    # model_path1 = '/data/b/shiwensong/project/malwareGA/main_attack/430features_6yearsdataset_all/RandomForest.pkl'
    # model_path2 = '/data/b/shiwensong/project/malwareGA/main_attack/430features_6yearsdataset_all/MLP_RandomForest.h5'
    # model_path3 = '/data/b/shiwensong/project/malwareGA/main_attack/430features_6yearsdataset_all/AdaBoost.pkl'
    # model_path4 = '/data/b/shiwensong/project/malwareGA/main_attack/430features_6yearsdataset_all/MLP_AdaBoost.h5'
    # model_path5 = '/data/b/shiwensong/project/malwareGA/main_attack/430features_6yearsdataset_all/AdaBoost.pkl'
    # model_path6 = '/data/b/shiwensong/project/malwareGA/main_attack/430features_6yearsdataset_all/MLP_AdaBoost.h5'
    # model_path7 = '/data/b/shiwensong/project/malwareGA/main_attack/430features_6yearsdataset_all/knn_3.pkl'
    # model_path8 = '/data/b/shiwensong/project/malwareGA/main_attack/430features_6yearsdataset_all/MLP_knn_3.h5'
