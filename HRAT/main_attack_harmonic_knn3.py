...
import myenv_withconstraints_harmonic_knn3
import os
import time
...
if __name__ == '__main__':
    ...
    seq_save_path = save_path + "/harmonic_actionseq"
    ...
    for zidx in tqdm(range(0, len(test_sha256_list))):
        try:
            ...
            env = myenv_withconstraints_harmonic_knn3.CFGModifierEnvConstraints(target_graph=X_test_triple,
                                                                  label=y_test,
            ...
            harmonic = env.getHarmonicCentrality(tmp_graph, X_test_sen_idx)
            non_zero = torch.nonzero(harmonic)
            print('non_zero', non_zero.size())
            cur_label, min_dist = env.getlabel(harmonic)
